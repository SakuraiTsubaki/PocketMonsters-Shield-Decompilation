...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hash every file in a local extracted game tree and emit deterministic JSON."
    )
    parser.add_argument("root", type=Path, help="Root directory to inventory")
    parser.add_argument(
        "--label",
        default="local-extraction",
        help="Non-sensitive label stored in the manifest instead of the absolute path",
    )
    parser.add_argument(
        "--output",
        type=Path,
